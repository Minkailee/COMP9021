...
class Maze():
    ...
    def getGates(self):
        ...
        return self.gates
    ##get the available ways of each cell
    def ways(self):
        ways=[]
        for i in range(self.row):
            eachl=[]
...
for i in wall:
    if len(i)!=col+1:
        print('Incorrect input.')
        exit()
if row+1>41 or row+1<2 or col+1>31 or col+1<2:
    print('Incorrect input.')
    exit()
...
ways1=maze2.ways()
areaNum=0
##find all the accessible areas.
##start from the remained gates.
##count the minimalist number of the gates that are able to get to every reachable cell.
    ...
    for i in range(maze.row):
        for j in range(maze.col):
            if maze2.isGate(i,j):
                ...
                #print(i,j,'s')
                explore(i,j)
##explore everyr reachable cell and if reach a gate close it.                
def explore(i,j):
    if ways1[i][j]==1:
        #print(i,j,'b')
        ways1[i][j]=-1
...
inaccessible=[]
##collect the inaccessible points
...
#print(Numinacc)
#print(acc_areas)
#find the entry-exit paths with no intersections not to cul-de-sacs.
maze5=deepcopy(maze)
way5=maze5.ways()
scaned5=scan(way5)
...
for i in range(maze.row):
        for j in range(maze.col):
            if scaned5[i][j]==-1:
                if maze5.maze[i][j].up==True \
                   and (i-1>=0 and i<maze.row and j>=0 and j<maze.col) \
                   and scaned5[i-1][j]!=-1:
                    maze5.maze[i-1][j].down=False
                if maze5.maze[i][j].down==True \
                   and (i>=0 and i+1<maze.row and j>=0 and j<maze.col) \
                   and scaned5[i+1][j]!=-1:
                    maze5.maze[i+1][j].up=False
                if maze5.maze[i][j].left==True \
                   and (i>=0 and i<maze.row and j-1>=0 and j<maze.col) \
                   and scaned5[i][j-1]!=-1:
                    maze5.maze[i][j-1].right=False
                if maze5.maze[i][j].right==True \
                   and (i>=0 and i<maze.row and j>=0 and j+1<maze.col) \
                   and scaned5[i][j+1]!=-1:
                    maze5.maze[i][j+1].left=False
#print(scaned5)
maze5.way=maze5.ways()
allroute=[]
route=[]    
        ...
        for j in range(maze.col):
            if ((i==0 and j==0) and maze.way[i][j]==2 and
               ...
               (maze.maze[i][j].down==True and maze.maze[i][j].right==True)):
                allroute.append([(i,j)])
               # print(i,j,'end')
            elif (maze.col==1 and (maze1.isGate(i,j) \
                   and maze5.maze[i][j].right==True \
                   and maze5.maze[i][j].left==True\
                   and maze5.maze[i][j].up==False \
                   and maze5.maze[i][j].down==False)):
                    #print(i,j,'r&l')
                    allroute.append([(i,j)])
            elif (maze.row==1 and (maze1.isGate(i,j) \
                    and maze5.maze[i][j].right==False \
                   and maze5.maze[i][j].left==False\
                   and maze5.maze[i][j].up==True \
                   and maze5.maze[i][j].down==True)):
                    #print(i,j,'u&d')
                    allroute.append([(i,j)])
            elif maze1.isGate(i,j):
                route=[]
                route.append((i,j))
                #print(i,j,'start')
                explore_route(i,j)
def explore_route(i,j):
    ...
    if maze1.way[i][j]!=2:
        ...
        return
    if maze5.way[i][j]==2:
        if maze5.maze[i][j].up==True and (i-1>=0 and i-1<maze.row and j>=0 and j<maze.col) and maze5.way[i-1][j]==2:
            maze5.maze[i-1][j].down=False
        ...
        if maze5.maze[i][j].down==True and (i+1>=0 and i+1<maze.row and j>=0 and j<maze.col) and maze5.way[i+1][j]==2:
            maze5.maze[i+1][j].up=False
            if maze1.isGate(i+1,j):
                route.append((i+1,j))
                allroute.append(route)
        ...
        if maze5.maze[i][j].left==True and (i>=0 and i<maze.row and j-1>=0 and j-1 < maze.col) and maze5.way[i][j-1]==2:
            maze5.maze[i][j-1].right=False
        ...
        if maze5.maze[i][j].right==True and (i>=0 and i < maze.row and j+1>=0 and j+1<maze.col) and maze5.way[i][j+1]==2:
            maze5.maze[i][j+1].left=False
            if maze1.isGate(i,j+1):
                route.append((i,j+1))
                allroute.append(route)
...
if not isPrint:
    ...
    if len(allroute)==0:
        ...
        print('The maze has '+str(len(allroute))+' entry-exit paths with no intersections not to cul-de-sacs.')
    #print(allroute)
if isPrint:
